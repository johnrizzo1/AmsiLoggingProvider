...
HRESULT SampleAmsiProvider::Scan(_In_ IAmsiStream* stream, _Out_ AMSI_RESULT* result)
{
    ...
    // hash the payload and save it as that
    // don't save duplicates
    // write hash to eventlog

    // Create base folder if it doesn't exist
    wstring baseFolder = L"C:\\amsilogs";
    ...
    if (contentAddress)
    {
        ...
        // TODO Move this to the tracelogger but for PoC this is enough.
    ...
    else
    {
        ...
        for (ULONGLONG position = 0; position < contentSize; position += readSize)
        {
            ...
            if (SUCCEEDED(hr))
            {
                ...
                // TODO Move this to the tracelogger but for PoC this is enough.
                WriteFile(h, chunk, sizeof(chunk), NULL, nullptr);
            }
            else
            {
